...
package object chapter6 {
  ...
  def accept(input: Input): State[Machine, Unit] = for {
    m <- State.get
    _ <- judge(input, m)
  } yield ()

  def judge(input: Input, machine: Machine): State[Machine, Unit] = if(machine.candies <= 0) State.set(machine) else {
    input match {
      case Coin => if(machine.locked) State.set(machine.copy(locked = false, coins = machine.coins+1)) else State.set(machine)
      case Turn => if(machine.locked) State.set(machine) else State.set(machine.copy(locked = true, candies = machine.candies-1))
    }
  }

  def simulateMachine(inputs: List[Input]): State[Machine, (Int, Int)] = for{
    _ <- State.traverse(inputs)(accept _)
    m <- State.get
